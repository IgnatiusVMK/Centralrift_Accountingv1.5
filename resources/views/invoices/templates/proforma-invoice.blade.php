...
<body>
    <div class="invoice-container">
       ...
       <div class="invoice-details">
            <div>
                ...
                <p><b>Category:</b> {{ $invoice->items->first()->product->category->Category_Name ?? 'N/A' }}</p>
            </div>
        </div>
        ...
        <table style="text-align: left">
            <thead>
                <tr>
                    <th>Produce Item</th>
                    <th>Description of Produce</th>
                    {{-- <th>No. of Boxes</th> --}}
                    {{-- <th>Packaging</th> --}}
                    <th>Weight</th>
                    <th>Unit Price <b>{{ $invoice->currency }}</b> </th>
                    <th>Total Price <b>{{ $invoice->currency }}</b> </th>
                </tr>
            </thead>
            <tbody>
    @foreach($invoice->items->where('type', 'proforma') as $item)
        <tr>
            <td style="text-align: left;">{{ $item->product->Product_Name ?? '' }}</td>
            <td style="text-align: left;">{{ $item->description }}</td>
            {{-- <td style="text-align: right;">{{ $item->quantity ?? '' }}</td> --}}
            <td style="text-align: left;">{{ $item->weight}} Kgs</td>                                     
            <td style="text-align: left;">{{ $item->unit_price }}</td>
    ...
    <tr>
        ...
        <td style="text-align: left;">
            {{ number_format($invoice->items->where('type', 'proforma')->sum('total'), 2, '.', ',') }}
        </td>
    </tr>
</tbody>
